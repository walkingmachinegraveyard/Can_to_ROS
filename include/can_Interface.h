  ...
  class canInterface
  {
  	...
  	protected:
	    ...
	    mode_t m_current_mode[];
		   ...
		   * @return              -1 on error
		   */
		  virtual int can_acquire(can_t dev)=0;
		   ...
		   * @return              -1 on error
		   */
		  virtual int can_release(can_t dev)=0;
  	...
  	public:
	    ...
	    virtual uint8_t  init(bool recv_own_msgs)=0;
			...
			/**
			 * Use to know how many frame is receive
			 * @param[in] canID       CAN device to access
			 * @return renvoie le nombre de frame recue
			 */
			uint16_t	get_frame_count(can_t canID){return frame_cnt[canID];};

			uint16_t	get_frame_count(uint16_t canID){return this->get_frame_count((can_t)canID);};

			void reset_frame_count(can_t canID){frame_cnt[canID]=0;};
